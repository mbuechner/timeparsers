...
public class TimeParserNew {
    ...
    private static final Logger LOG = LoggerFactory.getLogger(TimeParserNew.class);

    /**
     * Private Constructor
     */
    private TimeParserNew() {
    }

    public static void init(String transformationProcessId, String transformationStatusId)
            throws Exception {
        if (rules == null) {
            File rulesFile = new File("conf/timeparser/rulesNew.csv");
    ...
    private static String transform(String input, Rule rule) throws TransformationException {
        ...
        InputParser inputParser
                = new InputParser(inputPattern, TimeParserNew.MONTH_REPLACEMENTS, TimeParserNew.WEEKDAY_REPLACEMENTS);
    ...
    private static List<Rule> findRulesForInput(String input) {
        for (Replacement replacement : TimeParserNew.MONTH_REPLACEMENTS) {
            input = input.replace(replacement.from, "MM");
        }

        for (Replacement replacement : TimeParserNew.WEEKDAY_REPLACEMENTS) {
